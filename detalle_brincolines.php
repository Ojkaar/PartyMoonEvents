...
if (isset($_GET['id'])) {
    ...
    $query_brincolines = "SELECT * FROM brincolines WHERE id = $id";
    $resultado_brincolines = ObtenerRegistros($query_brincolines);

   
    // Si no se encuentra el artículo en ninguna de las tablas
    if (
        empty($resultado_brincolines)
    ) {
...
if (!empty($resultado_brincolines)) {
    $detalle = $resultado_brincolines[0];
}
else {
