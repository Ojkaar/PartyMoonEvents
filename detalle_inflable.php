...
if (isset($_GET['id'])) {
    ...
    $query_inflables = "SELECT * FROM inflables WHERE id = $id";
    $resultado_inflables = ObtenerRegistros($query_inflables);

   
    // Si no se encuentra el artículo en ninguna de las tablas
    if (
        empty($resultado_inflables)
    ) {
...
if (!empty($resultado_inflables)) {
    $detalle = $resultado_inflables[0];
}
else {
