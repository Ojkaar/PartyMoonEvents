...
if (isset($_GET['id'])) {
    ...
    $query_mesas = "SELECT * FROM mesas WHERE id = $id";
    $resultado_mesas = ObtenerRegistros($query_mesas);

   
    // Si no se encuentra el artículo en ninguna de las tablas
    if (
        empty($resultado_mesas)
    ) {
...
if (!empty($resultado_mesas)) {
    $detalle = $resultado_mesas[0];
}
else {
