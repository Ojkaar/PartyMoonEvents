...
if (isset($_GET['id'])) {
    ...
    $query_postre = "SELECT * FROM postres WHERE id = $id";
    $resultado_postre = ObtenerRegistros($query_postre);

   
    // Si no se encuentra el artículo en ninguna de las tablas
    if (
        empty($resultado_postre)
    ) {
...
if (!empty($resultado_postre)) {
    $detalle = $resultado_postre[0];
}
else {
