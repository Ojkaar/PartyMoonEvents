...
if (isset($_GET['id'])) {
    ...
    $query_articulos = "SELECT * FROM articulos WHERE id = $id";
    $resultado_articulo = ObtenerRegistros($query_articulos);

   
    // Si no se encuentra el artículo en ninguna de las tablas
    if (
        empty($resultado_articulo)
    ) {
...
if (!empty($resultado_articulo)) {
    $detalle = $resultado_articulo[0];
}
else {
...
<div class="container mt-5 main-container">
    <div class="article-content">
        ...
        <!-- Botón de reserva -->
        <a href="cotizaciones.php" class="btn-custom">Cotizar</a>
    </div>
</div>
