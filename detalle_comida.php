...
if (isset($_GET['id'])) {
    ...
    $query_comida = "SELECT * FROM comidas WHERE id = $id";
    $resultado_comida = ObtenerRegistros($query_comida);

   
    // Si no se encuentra el artículo en ninguna de las tablas
    if (
        empty($resultado_comida)
    ) {
...
if (!empty($resultado_comida)) {
    $detalle = $resultado_comida[0];
}
else {
